...
class Dense():
    ...
    def initialize_parameters(self):
        ...
        W = np.random.uniform(-1*limit,limit,(self.n_y,self.n_x))
        '''
        W = []
        for i in range(self.n_x):
            if i==0:
                W = np.random.uniform(-1*limit,limit,(self.n_y,1))
            else:
                new = np.random.uniform(-1*limit,limit,(self.n_y,1))
                W = np.append(W,new,axis=1)
        '''
    ...
    def backward(self, dZ):
        ...
        ### PASTE YOUR CODE HERE ###
        ### START CODE HERE ###
        self.dW = 1/m*np.dot(dZ,np.transpose(A_prev))
        self.db = np.transpose(np.sum(dZ,axis=1)*1/m).reshape(self.parameters["b"].shape)
        #print(self.db)
